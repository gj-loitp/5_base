...
/**
 * Created by www.muathu@gmail.com on 8/2/2017.
 */
...
public class LOnlyPageViewPager extends ViewGroup {
    private static final String TAG = LOnlyPageViewPager.class.getSimpleName();
    ...
    /**
     * Callback3 interface for responding to changing state of the selected page.
     */
    public interface OnPageChangeListener {
         ...
         * @see LOnlyPageViewPager#SCROLL_STATE_IDLE
         * @see LOnlyPageViewPager#SCROLL_STATE_DRAGGING
         * @see LOnlyPageViewPager#SCROLL_STATE_SETTLING
         */
    ...
    public LOnlyPageViewPager(Context context) {
        super(context);
        initViewPager();
    }

    public LOnlyPageViewPager(Context context, AttributeSet attrs) {
        super(context, attrs);
        initViewPager();
    }
    ...
    public void setOffscreenPageLimit(int limit) {
        if (limit < DEFAULT_OFFSCREEN_PAGES) {
            LLog.d(TAG, "Requested offscreen page limit " + limit + " too small; defaulting to " + DEFAULT_OFFSCREEN_PAGES);
    ...
    void populate() {
        ...
        if (mPopulatePending) {
            if (DEBUG) LLog.d(TAG, "populate is pending, skipping for now...");
            return;
        }
        ...
        if (DEBUG) LLog.d(TAG, "populating: startPos=" + startPos + " endPos=" + endPos);
        ...
        for (int i = 0; i < mItems.size(); i++) {
            ...
            if ((ii.position < startPos || ii.position > endPos) && !ii.scrolling) {
                if (DEBUG) LLog.d(TAG, "removing: " + ii.position + " @ " + i);
                mItems.remove(i);
                i--;
                mAdapter.destroyItem(this, ii.position, ii.object);
                ...
                while (lastPos <= endPos && lastPos < ii.position) {
                    if (DEBUG) LLog.d(TAG, "inserting: " + lastPos + " @ " + i);
        ...
        if (lastPos < endPos) {
            ...
            while (lastPos <= endPos) {
                if (DEBUG) LLog.d(TAG, "appending: " + lastPos);
        ...
        if (DEBUG) {
            LLog.d(TAG, "Current page list:");
            for (int i = 0; i < mItems.size(); i++) {
                LLog.d(TAG, "#" + i + ": page " + mItems.get(i).position);
            }
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (child.getVisibility() != GONE) {
                if (DEBUG) LLog.d(TAG, "Measuring #" + i + " " + child
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != GONE && (ii = infoForChild(child)) != null) {
                ...
                if (DEBUG) LLog.d(TAG, "Positioning #" + i + " " + child + " f=" + ii.object
    ...
    public void computeScroll() {
        if (DEBUG) LLog.d(TAG, "computeScroll: finished=" + mScroller.isFinished());
        if (!mScroller.isFinished()) {
            if (mScroller.computeScrollOffset()) {
                if (DEBUG) LLog.d(TAG, "computeScroll: still scrolling");
                int oldX = getScrollX();
                int oldY = getScrollY();
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_CANCEL || action == MotionEvent.ACTION_UP) {
            // Release the drag.
            if (DEBUG) LLog.d(TAG, "Intercept done!");
        ...
        if (action != MotionEvent.ACTION_DOWN) {
            if (mIsBeingDragged) {
                if (DEBUG) LLog.d(TAG, "Intercept returning true!");
                return true;
            }
            if (mIsUnableToDrag) {
                if (DEBUG) LLog.d(TAG, "Intercept returning false!");
                return false;
            }
        }
        ...
        switch (action) {
            case MotionEvent.ACTION_MOVE: {
                ...
                */
                ...
                if (DEBUG)
                    LLog.d(TAG, "Moved x to " + x + "," + y + " diff=" + xDiff + "," + yDiff);
                ...
                if (xDiff > mTouchSlop && xDiff > yDiff) {
                    if (DEBUG) LLog.d(TAG, "Starting drag!");
                    ...
                    if (yDiff > mTouchSlop) {
                        ...
                        if (DEBUG) LLog.d(TAG, "Starting unable to drag!");
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                if (DEBUG) LLog.d(TAG, "Down at " + mLastMotionX + "," + mLastMotionY
