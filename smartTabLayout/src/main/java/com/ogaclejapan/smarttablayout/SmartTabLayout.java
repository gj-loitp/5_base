...
public class SmartTabLayout extends HorizontalScrollView {
    ...
    public void setTabStateListener(TabStateListener tabStateListener) {
    ...
    private class InternalViewPagerListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            ...
            if (tabStateListener != null) {
                boolean hideLeft = false;
                boolean hideRight = false;

                Rect scrollBounds = new Rect();
                tabStrip.getDrawingRect(scrollBounds);
                ...
                view = tabStrip.getChildAt(tabStrip.getChildCount() - 1);
                int[] location = {0, 0};
                view.getLocationOnScreen(location);
                if (location[0] + view.getWidth() <= SmartTabLayout.this.getWidth()) {
                    //
                    hideRight = true;
                }
                tabStateListener.onFirstTabStateChange(hideLeft);
                tabStateListener.onLastTabStateChange(hideRight);
            }
        }
