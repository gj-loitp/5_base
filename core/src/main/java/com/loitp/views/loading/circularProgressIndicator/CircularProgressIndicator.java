...
public class CircularProgressIndicator extends View {
    ...
    private void init(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            progressColor = a.getColor(R.styleable.CircularProgressIndicator_cpiProgressColor, progressColor);
            progressBackgroundColor = a.getColor(R.styleable.CircularProgressIndicator_cpiProgressBackgroundColor, progressBackgroundColor);
            progressStrokeWidth = a.getDimensionPixelSize(R.styleable.CircularProgressIndicator_cpiProgressStrokeWidth, progressStrokeWidth);
            progressBackgroundStrokeWidth = a.getDimensionPixelSize(R.styleable.CircularProgressIndicator_cpiProgressBackgroundStrokeWidth, progressStrokeWidth);
            textColor = a.getColor(R.styleable.CircularProgressIndicator_cpiTextColor, progressColor);
            textSize = a.getDimensionPixelSize(R.styleable.CircularProgressIndicator_cpiTextSize, textSize);

            shouldDrawDot = a.getBoolean(R.styleable.CircularProgressIndicator_cpiDrawDot, true);
            dotColor = a.getColor(R.styleable.CircularProgressIndicator_cpiDotColor, progressColor);
            dotWidth = a.getDimensionPixelSize(R.styleable.CircularProgressIndicator_cpiDotWidth, progressStrokeWidth);

            startAngle = a.getInt(R.styleable.CircularProgressIndicator_cpiStartAngle, DEFAULT_PROGRESS_START_ANGLE);
            if (startAngle < 0 || startAngle > 360) {
                startAngle = DEFAULT_PROGRESS_START_ANGLE;
            }

            isAnimationEnabled = a.getBoolean(R.styleable.CircularProgressIndicator_cpiEnableProgressAnimation, true);
            isFillBackgroundEnabled = a.getBoolean(R.styleable.CircularProgressIndicator_cpiFillBackground, false);

            direction = a.getInt(R.styleable.CircularProgressIndicator_cpiDirection, DIRECTION_COUNTERCLOCKWISE);

            int cap = a.getInt(R.styleable.CircularProgressIndicator_cpiProgressCap, CAP_ROUND);
            progressStrokeCap = (cap == CAP_ROUND) ? Paint.Cap.ROUND : Paint.Cap.BUTT;

            String formattingPattern = a.getString(R.styleable.CircularProgressIndicator_cpiFormattingPattern);
            ...
            final int gradientType = a.getColor(R.styleable.CircularProgressIndicator_cpiGradientType, 0);
            if (gradientType != NO_GRADIENT) {
                final int gradientColorEnd = a.getColor(R.styleable.CircularProgressIndicator_cpiGradientEndColor, -1);
