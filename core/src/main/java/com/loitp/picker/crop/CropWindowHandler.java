...
final class CropWindowHandler {
     ...
     */
    public CropWindowMoveHandler getMoveHandler(float x, float y, float targetRadius, LCropImageView.CropShape cropShape) {
        CropWindowMoveHandler.Type type = cropShape == LCropImageView.CropShape.OVAL
    ...
    private CropWindowMoveHandler.Type getOvalPressedMoveType(float x, float y) {
        ...
        */
        ...
        final float cellLength = mEdges.width() / 6;
        final float leftCenter = mEdges.left + cellLength;
        final float rightCenter = mEdges.left + (5 * cellLength);

        final float cellHeight = mEdges.height() / 6;
        final float topCenter = mEdges.top + cellHeight;
        final float bottomCenter = mEdges.top + 5 * cellHeight;
