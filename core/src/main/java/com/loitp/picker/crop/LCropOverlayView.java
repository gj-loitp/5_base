...
public class LCropOverlayView extends View {
     ...
     */
    private LCropImageView.Guidelines mGuidelines;

    /**
     * The shape of the cropping area - rectangle/circular.
     */
    private LCropImageView.CropShape mCropShape;
    ...
    public void setBounds(float[] boundsPoints, int viewWidth, int viewHeight) {
        if (boundsPoints == null || !Arrays.equals(mBoundsPoints, boundsPoints)) {
            ...
            final RectF cropRect = mCropWindowHandler.getRect();
     ...
     * The shape of the cropping area - rectangle/circular.
     */
    public LCropImageView.CropShape getCropShape() {
        return mCropShape;
    }
     ...
     */
    public LCropImageView.Guidelines getGuidelines() {
        return mGuidelines;
    }
     ...
     */
    public void setGuidelines(LCropImageView.Guidelines guidelines) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCropWindowHandler.showGuidelines()) {
            ...
            if (mGuidelines == LCropImageView.Guidelines.ON) {
                drawGuidelines(canvas);
            } else if (mGuidelines == LCropImageView.Guidelines.ON_TOUCH && mMoveHandler != null) {
...
//                canvas.clipPath(mPath);
    ...
    private void drawCornerCircle(Canvas canvas) {
        if (mCirclePaint != null) {
            ...
            final float circleOffset = mCirclePaint.getStrokeWidth() / 2;
    ...
    private void drawCorners(Canvas canvas) {
        if (mBorderCornerPaint != null) {
            ...
            final float cornerOffset = (cornerWidth - lineWidth) / 2;
            final float cornerExtension = cornerWidth / 2 + cornerOffset;
    ...
    private boolean calculateBounds(RectF rect) {
        ...
        if (!isNonStraightAngleRotated()) {
            ...
            final float a0 = (y3 - y0) / (x3 - x0);
            final float a1 = -1f / a0;
            final float b0 = y0 - a0 * x0;
            final float b1 = y0 - a1 * x0;
            final float b2 = y2 - a0 * x2;
            final float b3 = y2 - a1 * x2;

            final float c0 = (rect.centerY() - rect.top) / (rect.centerX() - rect.left);
            final float c1 = -c0;
            final float d0 = rect.top - c0 * rect.left;
            final float d1 = rect.top - c1 * rect.right;
    ...
    private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            final float dY = detector.getCurrentSpanY() / 2;
            final float dX = detector.getCurrentSpanX() / 2;

            final float newTop = y - dY;
            final float newLeft = x - dX;
            final float newRight = x + dX;
            final float newBottom = y + dY;
