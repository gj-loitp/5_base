...
final class BitmapUtils {
    ...
    public static BitmapSampled decodeSampledBitmap(Context context, Uri uri, int reqWidth, int reqHeight) {
        try {
            ...
            final BitmapFactory.Options options = decodeImageForOption(resolver, uri);
            ...
            final Bitmap bitmap = decodeImage(resolver, uri, options);
    ...
    public static Bitmap cropBitmap(Bitmap bitmap, float[] points, int degreesRotated,
        ...
        final Rect rect = getRectFromPoints(points, bitmap.getWidth(), bitmap.getHeight(), fixAspectRatio, aspectRatioX, aspectRatioY);
    ...
    public static Rect getRectFromPoints(float[] points, int imageWidth, int imageHeight, boolean fixAspectRatio, int aspectRatioX, int aspectRatioY) {
        final int left = Math.round(Math.max(0, getRectLeft(points)));
        final int top = Math.round(Math.max(0, getRectTop(points)));
        final int right = Math.round(Math.min(imageWidth, getRectRight(points)));
        final int bottom = Math.round(Math.min(imageHeight, getRectBottom(points)));
     ...
     */
    public static Bitmap resizeBitmap(Bitmap bitmap, int reqWidth, int reqHeight, LCropImageView.RequestSizeOptions options) {
        try {
            if (reqWidth > 0 && reqHeight > 0 && (options == LCropImageView.RequestSizeOptions.RESIZE_FIT ||
                    options == LCropImageView.RequestSizeOptions.RESIZE_INSIDE ||
                    options == LCropImageView.RequestSizeOptions.RESIZE_EXACT)) {

                Bitmap resized = null;
                if (options == LCropImageView.RequestSizeOptions.RESIZE_EXACT) {
                    resized = Bitmap.createScaledBitmap(bitmap, reqWidth, reqHeight, false);
                } else {
                    final int width = bitmap.getWidth();
                    final int height = bitmap.getHeight();
                    final float scale = Math.max(width / (float) reqWidth, height / (float) reqHeight);
                    if (scale > 1 || options == LCropImageView.RequestSizeOptions.RESIZE_FIT) {
    ...
    private static BitmapSampled cropBitmap(Context context, Uri loadedImageUri, float[] points,
        ...
        // get the rectangle in original image that contains the required cropped area (larger for non rectangular crop)
        final Rect rect = getRectFromPoints(points, orgWidth, orgHeight, fixAspectRatio, aspectRatioX, aspectRatioY);

        final int width = reqWidth > 0 ? reqWidth : rect.width();
        final int height = reqHeight > 0 ? reqHeight : rect.height();

        Bitmap result = null;
        int sampleSize = 1;
        try {
            // decode only the required image from URI, optionally sub-sampling if reqWidth/reqHeight is given.
            final BitmapSampled bitmapSampled = decodeSampledBitmapRegion(context, loadedImageUri, rect, width, height, sampleMulti);
    ...
    private static BitmapSampled cropBitmap(Context context, Uri loadedImageUri, float[] points,
        ...
        try {
            ...
            final Bitmap fullBitmap = decodeImage(context.getContentResolver(), loadedImageUri, options);
            if (fullBitmap != null) {
                try {
                    // adjust crop points by the sampling because the image is smaller
                    final float[] points2 = new float[points.length];
    ...
    private static Bitmap cropForRotatedImage(Bitmap bitmap, float[] points, Rect rect, int degreesRotated,
        ...
        if (degreesRotated % 90 != 0) {
            ...
            final double rads = Math.toRadians(degreesRotated);
            final int compareTo = degreesRotated < 90 || (degreesRotated > 180 && degreesRotated < 270) ? rect.left : rect.right;
