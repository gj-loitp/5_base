...
public class DraggablePanelFreeLayout extends ViewGroup {
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            LLog.INSTANCE.d(TAG, "onViewPositionChanged " + left + ", " + top + ", " + dx + ", " + dy + ", " + mDragOffset);
            if (mDragOffset == 0f) {
                if (mState != State.TOP) {
                    mState = State.TOP;
                    if (callback != null) {
                ...
                if (mState != State.BOTTOM) {
                    ...
                    LLog.INSTANCE.d(TAG, "State.BOTTOM");
                }
            } else {
                if (mState != State.MID) {
                    mState = State.MID;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEventCompat.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (isPositionBottom()) {
                    LLog.INSTANCE.d(TAG, "onTouchEvent ACTION_UP no slide");
                } else {
                    if (mDragOffset < 0.5f) {
                        maximize();
                    } else {
